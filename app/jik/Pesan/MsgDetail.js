...
export default class MsgDetail extends Component{
	...
	ListNew(){
		...
		if (this.props.data !== false && typeof this.props.data !== "undefined" && this.props.data !== "") {
			...
			//pesan saya masuk
			if (this.props.status !== false && typeof this.props.status !== "undefined" && this.props.status !== "") {
				if (this.props.status == "masuk") {
					let dateTime = <View/>
					...
					return (
							<View>
								<View style={{ padding:15, paddingTop:0, paddingBottom:0, backgroundColor:'#F0F0F0', }}>
									<TouchableHighlight
									onPress={()=>this._Actions('detailmsg',{status:this.props.status})}>
										...
										<View style={{ flexDirection:'row', borderTopWidth:1, borderColor:'#F5F5F5', marginTop:15, paddingTop:15, paddingBottom:15, }}>
											...
											<View style={{ flexDirection:'row', justifyContent:'flex-end', alignItems:'flex-end', }}>
												<View style={{ borderColor:'#F5F5F5', borderRightWidth:1, }}>
													<TouchableHighlight
													onPress={()=>this._Actions('replymsg',{status:this.props.status})}>
				...
				}else if(this.props.status == "terkirim") {
					console.log("msg Detail => ",this.props.data);
					this.ActionButton.formatDate(
						{
							dmy:msg.senderDate,
					...
					return (<View>
								...
								<View style={{ marginBottom:15, paddingLeft:0, paddingRight:0, }}>
									<View style={ this.stylewrapper }>
										<View style={{ flexDirection:'row', borderTopWidth:1, borderColor:'#F5F5F5', marginTop:15, paddingTop:15, paddingBottom:15, }}>
											<View style={{ flex:1, flexDirection:'row', alignItems: 'center' }}>
												<Text style={{ fontSize:14, }}>{ msg.typeData+" : "+msg.toEmail }</Text>
											</View>
											<View style={{ flexDirection:'row', justifyContent:'flex-end', alignItems:'flex-end', }}>
												<View style={{ }}>
													<TouchableHighlight
													onPress={()=>{
														this._Actions('remove',{status:this.props.status, id:this.props.data.msg.id,});
													}}>
														<View style={{ padding:2.5, width:this.dim8, height:this.dim8, borderRadius:this.dim8, flexDirection: 'row', justifyContent:'center',alignItems:'center', backgroundColor:'#fff', }}>
															<IonIcon name="ios-trash-outline" style={{ fontSize:20, }}/>
														</View>
									...
									<View style={[this.stylewrapper,{ backgroundColor:'#e7e7e7', paddingBottom:1,  }, ]}>
										<View style={{ paddingBottom:15, paddingTop:15, }}>
											<Text style={{ lineHeight:25, }}>{msg.senderBody}</Text>
										</View>
									</View>
								</View>
							</View>);
				}else if(this.props.status == "sampah") {
					this.ActionButton.formatDate(
						{
							dmy:msg.date,
							hhmmss:msg.date
												...
												<View style={{ flexDirection:'row', justifyContent:'flex-end', alignItems:'flex-end', }}>
													...
													<View>
														<View style={{ padding:2.5, paddingRight:0, paddingLeft:10, flexDirection: 'row', justifyContent:'center',alignItems:'center', }}>
															<IonIcon name="ios-arrow-forward-outline" style={{ fontSize:20, }}/>
														</View>
													</View>
												</View>
											</View>
