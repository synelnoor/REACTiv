	...
	_getList(){
		...
		newComFetch.sendFetch((rsp) => {
			if(rsp.status === 200){
				...
				for(i in data){
					compList.push(<ComList key={i} data={data[i]} from={'gik'} gotoContent={(loop)=>(this._gotoContent(loop))} setHeight={(loop,height)=>(this._setHeight(loop,height))}/>);
				}
