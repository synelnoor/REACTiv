...
export default class ComHomeBanner extends Component{
	...
	_Actions(data){
		...
		if(data.apps_action === 'open_external_link' && data.apps_value !== ''){
			...
			Actions.GIKArticleList(JSON.parse(data.apps_value));
		}
		else if(data.apps_action === 'detail_article' && data.data){
			Actions.GIKArticleDetail(data.data);
		}
		else if(data.apps_action === 'album_photo' && data.data){
			Actions.GIKPhotoAlbum(data.data);
		}
		else if(data.apps_action === 'album_video' && data.data){
			Actions.GIKVideoAlbum(data.data);
		}
		else if(data.apps_action === 'album_video' && data.apps_value !== ''){
			Actions.GIKVideoAlbum(JSON.parse(data.apps_value));
		}
		else if(data.apps_action === 'kegiatan' && data.apps_value !== ''){
			// Actions.GIKKegiatan({'ymd':data.apps_value}); commented by dandi
			Actions.GIKKegiatan();
		}
		else if(data.apps_action === 'streaming'){
			Actions.GIKStreaming();
		}
		else{
			return;
		}
	...
	_getBanner(){
		...
		query['table'] = 'gik_rotate_banner';
		query['where[trash_status#=]'] = 'N';
		query['where[show_in#!=]'] = 'web';
		query['orderBy[order_id]'] = 'ASC';
		query['injected[gik_home_big_rotate_banner]'] = true;
		query['injected[gik_home_big_rotate_banner][dimensions_width]'] = this.state.dimensions.width*2;
