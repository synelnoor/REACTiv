...
export default class ModalReservation extends Component {
		...
		if(!this.state.modalActive){
			...
			LocalStorage.getItemByKey('@jwt:user_info',(e)=>{
				...
				this.setState({
					...
					console.log('token',this.jwt_signature)
					console.log('queryReservasi',resource)
	...
	_modal(){
		if(this.state.formReserv !== null){
			console.log('cekKuota',this.state.formReserv);

			let formReserv = this.state.formReserv;
			let note = <View/>
			let content = <View/>
			console.log('formremaining',formReserv.remaining_quota)
			...
			if(typeof formReserv.message !== 'undefined' || typeof formReserv.message.string !== 'undefined' || formReserv.message.string !== ''){
				...
				// added by dandi 20180824
					...
					console.log('nullQouta')
					ms='<div style="text-align:center"><strong>Maaf,<br>reservasi online '+ formReserv.row.title+'<br>sudah ditutup.<br>Silahkan datang langsung 1 jam sebelum pertunjukan dimulai untuk registrasi daftar tunggu ke :<br>Galeri Indonesia Kaya - Grand Indonesia West Mall lantai 8</strong></div>'
					note = <View>
						{/* <WebHtmlView source={{html:'<style>*{margin:0 !important;padding:0 !important;color:#555 !important;font-size:13px;font-weight:normal;}strong{font-weight:bold !important;}</style>'+formReserv.message.string}}/> */}
						<WebHtmlView source={{html:'<style>*{margin:0 !important;padding:0 !important;color:#555 !important;font-size:13px;font-weight:normal;}strong{font-weight:bold !important;}</style>'+ms}}/>
					</View>
				}
				else if(formReserv.status === 'tutup'){
					if(formReserv.row.type === 'gik'){
						console.log('Reservasigik')
						note = <View>
							<WebHtmlView source={{html:'<style>*{margin:0 !important;padding:0 !important;color:#555 !important;font-size:13px;font-weight:normal;}strong{font-weight:bold !important;}</style>'+formReserv.message.string}}/>
						</View>;
					}
					else{
						console.log('allElseReservasigik')
						content = <View>
