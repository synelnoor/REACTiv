...
export default class Album extends Component{
	...
	_getGrid(){
		...
		if(temp.length > 0){
			for(i in temp){
				let a = i;
				compList.push(<ComList key={i} data={temp[i]} from={'ik'} showModal={(img,title)=>{this._showModal(img,title,a)}}/>);
			}
			compList.push(
		...
		newComFetch.sendFetch((rsp) => {
			...
			if(rsp.status === 200){
				...
				for(i in newTemp){
					let a = i;
					compList.push(<ComList key={i} data={newTemp[i]} from={'ik'} showModal={(img,title)=>{this._showModal(img,title,a)}}/>);
				}
		...
		if(
			...
			return(
				<TouchableHighlight
					...
					onPress={()=>Actions.IKArticleDetail(this.props.data)}
				>
